...
int main(){
    ...
    cin >> n;
    vector<int> arr(n); // Example: 4, 2, 2, 8, 3, 3, 1
    cout << "Enter elements: " << endl;
    // Counting sort works best when values are integers in a small range.
    ...
    // Build a frequency table for the value range [minValue, maxValue].
    ...
    for(int i = 1; i < count.size(); i++){
        // Convert frequency to prefix sums -> each entry tells the "end position" for a value.
        count[i] += count[i - 1];
    } // count = {1, 3, 5, 6, 6, 6, 6, 7}
    vector<int> output(n);
    // vector output has size of n = 7 elements
    for(int i = n - 1; i >= 0; i--){
        // Stable placement: walk input from right to left.
