CREATE TABLE drug (
    drug_name VARCHAR(120) PRIMARY KEY
);
...
CREATE TABLE disease (
    disease_name VARCHAR(200) PRIMARY KEY,
    category_name VARCHAR(120) NOT NULL,
    FOREIGN KEY (category_name) REFERENCES disease_category(category_name)
);

CREATE TABLE side_effect (
    side_effect_name VARCHAR(120) PRIMARY KEY
);
...
CREATE TABLE company (
    company_name VARCHAR(160) PRIMARY KEY
);
...
CREATE TABLE clinical_trial (
    trial_title VARCHAR(255) PRIMARY KEY,
    start_date DATE,
...
CREATE TABLE condition (
    condition_name VARCHAR(200) PRIMARY KEY
);
